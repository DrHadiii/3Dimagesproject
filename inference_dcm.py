...
def get_predicted_volumes(pred):
    ...
    volume_ant=np.sum(np.where(pred==1))
    volume_post=np.sum(np.where(pred==2))
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90),f"Patient ID: {header.PatientID}\n Anterior Hippocampus Volume: {inference['anterior']}\n Posterior Hippocampus Volume: {inference['posterior']}",(255, 255, 255),
                        font=main_font)
