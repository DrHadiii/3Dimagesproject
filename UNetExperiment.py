...
class UNetExperiment:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # ANSWER: probability that voxel is of a certain class 
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.val_loader):
                ...
                data_val=batch["image"].to(self.device,dtype=torch.float)
                target_val=batch["seg"].to(self.device)
                prediction=self.model(data_val)
                prediction_softmax=F.softmax(prediction,dim=1)
                loss=self.loss_function(prediction_softmax, target_val[:, 0, :, :])

                print(f"Batch {i}. Data shape {data_val.shape} Loss {loss}")
